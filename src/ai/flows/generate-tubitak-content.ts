...
const prompt = ai.definePrompt({
  ...
  **For "Tasarım" (Design) projects, use this structure:**
  ...
  - **Yöntem:** Detail the process with clear steps. Examples include: 1. Problemi araştırma (Problem Research). 2. Gereksinimleri belirleme (Requirement Specification). 3. Olası çözümleri geliştirme (Developing Solutions). 4. En iyi çözümü seçme (Selecting the Best Solution). 5. Prototip oluşturma/yapılandırma (Prototyping/Configuration). 6. Çözümleri test etme ve değerlendirme (Testing and Evaluation). Describe the technology (e.g., Arduino, sensors, 3D printing, coding) and materials used. For example: "Mekanizmanın yapımında düz dişli çarklar, paslanmaz vidalı kesme bıçakları kullanılacaktır. Tasarım modellemesini bilgisayar ortamında yaptıktan sonra talaşlı imalat yöntemleri ile makine parçalarının imalatını yapacağız."
