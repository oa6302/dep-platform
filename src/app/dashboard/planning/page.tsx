    ...
    <div className="p-4 md:p-10 space-y-12 max-w-[1800px] mx-auto w-full animate-in fade-in duration-1000 bg-[#F8FAFC]">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-10">
        ...
        <div className="flex flex-col gap-6 w-full xl:w-auto">
          <Card className="p-8 rounded-[3.5rem] border-none shadow-[0_40px_80px_-20px_rgba(15,23,42,0.12)] bg-white flex flex-wrap gap-8 items-end justify-center md:justify-start relative overflow-hidden group">
             <div className="absolute top-0 right-0 w-32 h-32 bg-accent/5 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2" />
             
             <div className="space-y-2 relative z-10">
                <Label className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 ml-4 italic text-primary">BAŞLANGIÇ</Label>
                <Input 
                  type="date" 
                  value={startDate} 
                  onChange={(e) => { setStartDate(e.target.value); setViewMode('daily'); }} 
                  className="h-16 w-[200px] rounded-2xl bg-slate-50 border-none font-bold text-base px-6 shadow-inner focus-visible:ring-accent transition-all" 
                />
             </div>

             <div className="space-y-2 relative z-10">
                <Label className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 ml-4 italic text-primary">BİTİŞ</Label>
                <Input 
                  type="date" 
                  value={endDate} 
             ...
             <Button 
               ...
               className="h-16 px-10 rounded-[1.75rem] bg-primary hover:bg-accent text-white font-black text-[11px] uppercase tracking-[0.2em] gap-4 shadow-[0_20px_40px_-10px_rgba(15,23,42,0.3)] transition-all active:scale-95 relative z-10 border-none"
             >
                {isRegenerating ? <Loader2 className="h-5 w-5 animate-spin" /> : <Sparkles className="h-5 w-5 animate-pulse text-accent" />} RAPORU ÇALIŞTIR
             </Button>
          </Card>
          
          <div className="flex flex-wrap gap-2">
             <div className="flex gap-2 bg-white p-2.5 rounded-2xl border border-primary/5 shadow-lg overflow-x-auto scrollbar-hide max-w-full">
                {['today', 'week', 'month', 'year'].map(f => (
                  <button key={f} onClick={() => handleQuickFilter(f)} className="h-12 px-5 rounded-xl font-black text-[9px] uppercase tracking-widest text-primary/40 hover:bg-slate-50 hover:text-primary transition-all">
             ...
             <div className="flex gap-2 bg-white p-2.5 rounded-2xl border border-primary/5 shadow-lg overflow-x-auto scrollbar-hide max-w-full">
                {academicMonths.map((m, i) => (
                  <button 
                    ...
                    className={cn(
                      "h-12 min-w-[90px] px-5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all whitespace-nowrap flex items-center justify-center gap-2", 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
         {[
           ...
           <Card key={i} className="premium-card p-6 border border-primary/5 group relative overflow-hidden bg-white rounded-[2.5rem]">
               ...
               return (
                <Card key={i} onClick={() => { setSelectedMonth(m.date); setViewMode('monthly'); }} className="p-8 rounded-[3rem] bg-white border border-primary/5 shadow-lg group hover:scale-[1.02] transition-all cursor-pointer">
                   <div className="flex justify-between items-center mb-6">
                      <div className="flex items-center gap-4">
                         <span className="text-3xl font-black text-primary italic tracking-tighter">{m.label}</span>
                   ...
                   <div className="mt-4 flex justify-between text-[9px] font-black uppercase tracking-widest text-primary/30 italic">
                 ...
                 {day.date === '2026-12-01' && (
                    <Card className="p-10 rounded-[3.5rem] bg-accent text-primary border-none shadow-xl relative overflow-hidden group">
                       ...
                       <div className="flex items-center gap-8 relative z-10">
                          <div className="h-20 w-20 rounded-[2rem] bg-primary text-white flex items-center justify-center shadow-2xl shrink-0">
        ...
        <DialogContent className="rounded-[3.5rem] border-none shadow-2xl p-10 bg-white max-w-xl overflow-hidden">
           ...
           {editingBlock && (
             <div className="space-y-8 pt-6">
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase opacity-40 ml-4 italic">DURUM</Label>
                  <div className="flex gap-3">
                    <button 
                      onClick={() => setEditingBlock({...editingBlock, status: 'planned'})}
                      className={cn("flex-1 h-14 rounded-xl font-black uppercase text-[10px] transition-all", editingBlock.status === 'planned' ? "bg-primary text-white" : "bg-slate-50 text-primary/40")}
                    >BEKLEMEDE</button>
                    <button 
                      onClick={() => setEditingBlock({...editingBlock, status: 'done'})}
                      className={cn("flex-1 h-14 rounded-xl font-black uppercase text-[10px] transition-all", editingBlock.status === 'done' || editingBlock.status === 'completed' ? "bg-emerald-50 text-white" : "bg-slate-50 text-primary/40")}
                    >TAMAMLANDI</button>
                  </div>
                </div>
