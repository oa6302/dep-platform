  ...
  return (
    <div className="min-h-screen p-6 lg:p-14 space-y-12 max-w-[1600px] mx-auto w-full animate-in fade-in duration-1000 bg-[#F8FAFC]">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-0.5">
             ...
             <h2 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter italic text-primary uppercase leading-tight text-shadow-premium">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 items-start">
        {/* Ana Timer Kartı */}
        <Card className="xl:col-span-7 p-12 md:p-20 rounded-[4rem] border-none shadow-[0_80px_160px_-40px_rgba(15,23,42,0.15)] bg-white text-center space-y-16 relative overflow-hidden group">
           ...
           <div className="flex justify-center gap-6 relative z-10">
              <button 
                ...
                className={cn("px-10 py-3 rounded-2xl font-black text-[10px] uppercase tracking-[0.4em] transition-all duration-500", mode === 'work' ? 'bg-[#0F172A] text-white shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-30 hover:opacity-100')}
              >ÇALIŞMA</button>
              <button 
                onClick={() => {setMode('break'); resetTimer();}}
                className={cn("px-10 py-3 rounded-2xl font-black text-[10px] uppercase tracking-[0.4em] transition-all duration-500", mode === 'break' ? 'bg-accent text-primary shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-30 hover:opacity-100')}
              >KISA MOLA</button>
           </div>

           <div className="relative z-10 flex items-center justify-center gap-2 md:gap-6">
              <p className="text-[10rem] md:text-[14rem] lg:text-[18rem] font-black italic tracking-tighter text-[#0F172A] leading-none text-shadow-premium">
                 {String(minutes).padStart(2, '0')}
              </p>
              <div className="flex flex-col gap-3 py-6 px-3 bg-accent rounded-full shadow-[0_20px_40px_-10px_rgba(245,158,11,0.5)]">
                 <div className="h-3 w-3 md:h-4 md:w-4 rounded-full bg-[#0F172A]" />
                 <div className="h-3 w-3 md:h-4 md:w-4 rounded-full bg-[#0F172A]" />
              </div>
              <p className="text-[10rem] md:text-[14rem] lg:text-[18rem] font-black italic tracking-tighter text-[#0F172A] leading-none text-shadow-premium">
           ...
           <div className="flex justify-center gap-10 relative z-10 pt-4">
              <Button 
                onClick={toggleTimer}
                className={cn(
                  "h-32 w-32 rounded-full shadow-[0_40px_80px_-20px_rgba(15,23,42,0.4)] transition-all duration-500 hover:scale-110 active:scale-95 group/play", 
                  isActive ? 'bg-slate-100 text-primary' : 'bg-[#0F172A] text-white'
                )}
              >
                 {isActive ? <Pause className="h-12 w-12" /> : <Play className="h-12 w-12 ml-2 fill-current group-hover/play:animate-pulse" />}
              </Button>
              <Button 
                onClick={resetTimer}
                variant="outline"
                className="h-32 w-32 rounded-full border-[6px] border-slate-50 bg-white text-muted-foreground hover:text-accent hover:border-accent transition-all duration-500 hover:scale-110 shadow-xl active:scale-95"
              >
        ...
        <div className="xl:col-span-5 space-y-10">
           {/* Ayarlar Terminali */}
           <Card className="p-10 rounded-[3.5rem] border-none shadow-lg bg-white space-y-10 relative overflow-hidden group">
           ...
           {/* AI Analiz Terminali */}
           <Card className="p-10 rounded-[3.5rem] border-none shadow-xl bg-[#0F172A] text-white space-y-6 relative overflow-hidden group border-t-8 border-accent/20">
           ...
           {/* Müzik Terminali */}
           {showMusic && (
              <Card className="p-10 rounded-[4rem] border-none shadow-3xl bg-[#0F172A] text-white space-y-8 animate-in slide-in-from-right-8 duration-700 relative overflow-hidden">
                 ...
                 <div className="aspect-video w-full rounded-[2.5rem] bg-black/60 overflow-hidden relative shadow-3xl border border-white/5 group/video">
                    ...
                    <div className="absolute bottom-6 left-8 space-y-1">
                       ...
                       <p className="text-2xl font-black italic tracking-tighter uppercase">{selectedStream.title}</p>
                    </div>
                 </div>
