    ...
    <div className="p-8 lg:p-14 space-y-12 max-w-[1600px] mx-auto w-full animate-in fade-in duration-1000 bg-[#F8FAFC]">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        ...
        <div className="flex gap-4">
           <Button 
             ...
             className={cn(
               "h-16 px-8 rounded-2xl transition-all font-black text-[10px] uppercase tracking-widest gap-3 shadow-2xl",
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
        ...
        <Card className="xl:col-span-7 p-16 rounded-[5rem] border-none shadow-[0_60px_120px_-30px_rgba(15,23,42,0.15)] bg-white text-center space-y-16 relative overflow-hidden group">
           ...
           <div className="flex justify-center gap-4 relative z-10">
              <button 
                ...
                className={cn("px-12 py-4 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] transition-all duration-500", mode === 'work' ? 'bg-primary text-white shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-40 hover:opacity-100')}
              >ÇALIŞMA</button>
              <button 
                onClick={() => {setMode('break'); resetTimer();}}
                className={cn("px-12 py-4 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] transition-all duration-500", mode === 'break' ? 'bg-accent text-primary shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-40 hover:opacity-100')}
              >KISA MOLA</button>
           </div>

           <div className="relative z-10 py-10">
              <p className="text-[12rem] md:text-[18rem] font-black italic tracking-tighter text-primary leading-[0.8] text-shadow-premium">
              ...
              <div className="flex items-center justify-center gap-4 mt-12">
                 ...
                 <p className="text-[12px] font-black uppercase tracking-[0.8em] text-primary/20 italic">DERİN ODAKLANMA MODU</p>
                 <div className="h-px w-20 bg-slate-100" />
              </div>
           </div>

           <div className="flex justify-center gap-10 relative z-10 pt-6">
              <Button 
                onClick={toggleTimer}
                className={cn(
                  "h-32 w-32 rounded-full shadow-[0_40px_80px_-15px_rgba(0,0,0,0.2)] transition-all duration-500 hover:scale-110 active:scale-95", 
                  isActive ? 'bg-slate-100 text-primary' : 'bg-primary text-white shadow-primary/30'
                )}
              >
                 {isActive ? <Pause className="h-12 w-12" /> : <Play className="h-12 w-12 ml-2" />}
              </Button>
              <Button 
                onClick={resetTimer}
                variant="outline"
                className="h-32 w-32 rounded-full border-4 border-slate-50 bg-white text-muted-foreground hover:text-accent hover:border-accent transition-all duration-500 hover:scale-110 shadow-2xl active:scale-95"
              >
        ...
        <div className="xl:col-span-5 space-y-12">
           ...
           {showMusic && (
              <Card className="p-12 rounded-[4rem] border-none shadow-2xl bg-[#0F172A] text-white space-y-10 animate-in slide-in-from-right-8 duration-700 relative overflow-hidden">
                 ...
                 <div className="aspect-video w-full rounded-[2.5rem] bg-black/40 overflow-hidden relative shadow-inner border border-white/5">
                    <iframe 
                      width="100%" 
                      height="100%" 
                 ...
                 <div className="grid grid-cols-2 gap-4 relative z-10">
                    {LOFI_STREAMS.map((stream) => (
                       ...
                       >
                          <div className={cn("h-8 w-8 rounded-xl flex items-center justify-center text-white shadow-lg group-hover:rotate-12 transition-transform", stream.color)}>
                             <Youtube className="h-4 w-4" />
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-tight line-clamp-1">{stream.title}</span>
                       </button>
                    ))}
                 </div>
              </Card>
           ...
           {/* Ayarlar Terminali */}
           <Card className="p-12 rounded-[4rem] border-none shadow-xl bg-white space-y-10 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50 rounded-full translate-x-1/2 -translate-y-1/2" />
              <div className="flex items-center gap-4 relative z-10">
                 <Settings2 className="h-7 w-7 text-primary opacity-20" />
                 <h4 className="text-2xl font-black italic tracking-tighter uppercase text-primary">AYARLAR</h4>
              </div>
              
              <div className="grid grid-cols-2 gap-10 relative z-10">
                 <div className="space-y-4">
                    <Label className="text-[11px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 ml-4">ÇALIŞMA (DK)</Label>
                    <div className="relative group">
                       <Clock className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-primary/20 group-focus-within:text-accent transition-colors" />
                       <Input 
                         type="number" 
                         value={workMins} 
                         onChange={(e) => setWorkMinutes(Number(e.target.value))}
                         className="h-16 rounded-2xl bg-slate-50 border-none shadow-inner pl-14 font-black text-xl text-primary" 
                       />
                    </div>
                 </div>
                 <div className="space-y-4">
                    <Label className="text-[11px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 ml-4">MOLA (DK)</Label>
                    <div className="relative group">
                       <Coffee className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-primary/20 group-focus-within:text-accent transition-colors" />
                       <Input 
                         type="number" 
                         value={breakMins} 
              ...
              <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex items-center justify-between">
                 <div className="flex items-center gap-5">
                    <div className="h-12 w-12 rounded-2xl bg-white flex items-center justify-center shadow-lg"><Target className="h-6 w-6 text-accent" /></div>
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">GÜNLÜK HEDEF</p>
                       <p className="text-lg font-black text-primary italic">12 SEANS / 300 XP</p>
                    </div>
                 </div>
                 <div className="h-12 w-12 rounded-full bg-white flex items-center justify-center shadow-md font-black text-xs text-primary">75%</div>
              </div>
           </Card>

           {/* AI Analiz */}
           <Card className="p-12 rounded-[4rem] border-none shadow-xl bg-primary text-white space-y-8 relative overflow-hidden group">
