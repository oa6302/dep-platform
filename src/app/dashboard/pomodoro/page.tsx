      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-0.5">
             ...
             <h2 className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter italic text-primary uppercase leading-tight text-shadow-premium break-words max-w-full">
        ...
        <Card className="xl:col-span-7 p-10 md:p-16 rounded-[4rem] border-none shadow-[0_80px_160px_-40px_rgba(15,23,42,0.15)] bg-white text-center space-y-12 relative overflow-hidden group">
           ...
           <div className="flex justify-center gap-4 relative z-10">
              <button 
                ...
                className={cn("px-6 md:px-10 py-3 rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.4em] transition-all duration-500", mode === 'work' ? 'bg-[#0F172A] text-white shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-30 hover:opacity-100')}
              >ÇALIŞMA</button>
              <button 
                onClick={() => {setMode('break'); resetTimer();}}
                className={cn("px-6 md:px-10 py-3 rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.4em] transition-all duration-500", mode === 'break' ? 'bg-accent text-primary shadow-2xl scale-105' : 'bg-slate-50 text-muted-foreground opacity-30 hover:opacity-100')}
              >KISA MOLA</button>
           </div>

           <div className="relative z-10 flex items-center justify-center gap-1 md:gap-4 overflow-hidden">
              <p className="text-[7rem] sm:text-[10rem] md:text-[12rem] lg:text-[15rem] font-black italic tracking-tighter text-[#0F172A] leading-none text-shadow-premium">
                 {String(minutes).padStart(2, '0')}
              </p>
              <div className="flex flex-col gap-2 md:gap-3 py-4 md:py-6 px-2 md:px-3 bg-accent rounded-full shadow-[0_20px_40px_-10px_rgba(245,158,11,0.5)] shrink-0">
                 <div className="h-2 w-2 md:h-4 md:w-4 rounded-full bg-[#0F172A]" />
                 <div className="h-2 w-2 md:h-4 md:w-4 rounded-full bg-[#0F172A]" />
              </div>
              <p className="text-[7rem] sm:text-[10rem] md:text-[12rem] lg:text-[15rem] font-black italic tracking-tighter text-[#0F172A] leading-none text-shadow-premium">
           ...
           <div className="flex items-center justify-center gap-4 md:gap-6 relative z-10">
              ...
              <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] md:tracking-[0.6em] text-primary/10 italic whitespace-nowrap">DERİN ODAKLANMA MODU</p>
              <div className="h-px flex-1 bg-gradient-to-l from-transparent to-slate-200" />
           </div>

           <div className="flex justify-center gap-6 md:gap-10 relative z-10 pt-4">
              <Button 
                onClick={toggleTimer}
                className={cn(
              ...
              >
                 {isActive ? <Pause className="h-8 w-8 md:h-12 md:w-12" /> : <Play className="h-8 w-8 md:h-12 md:w-12 ml-1 md:ml-2 fill-current group-hover/play:animate-pulse" />}
              </Button>
              <Button 
                onClick={resetTimer}
                variant="outline"
                className="h-24 w-24 md:h-32 md:w-32 rounded-full border-[4px] md:border-[6px] border-slate-50 bg-white text-muted-foreground hover:text-accent hover:border-accent transition-all duration-500 hover:scale-110 shadow-xl active:scale-95"
              >
                 <RotateCcw className="h-8 w-8 md:h-12 md:w-12" />
              </Button>
           </div>
        </Card>
        ...
        <div className="xl:col-span-5 space-y-10">
           ...
           <Card className="p-8 md:p-10 rounded-[3.5rem] border-none shadow-lg bg-white space-y-10 relative overflow-hidden group">
              ...
              <div className="flex items-center gap-5 relative z-10">
                 ...
                 <h4 className="text-xl md:text-2xl font-black italic tracking-tighter uppercase text-primary">AYARLAR</h4>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-8 relative z-10">
                 <div className="space-y-3">
                    ...
                    <div className="relative group">
                       <Clock className="absolute left-5 md:left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-primary/20 group-focus-within:text-accent transition-colors" />
                       <Input 
                         type="number" 
                         value={workMins} 
                         onChange={(e) => setWorkMinutes(Number(e.target.value))}
                         className="h-14 md:h-16 rounded-2xl bg-slate-50 border-none shadow-inner pl-12 md:pl-14 font-black text-2xl md:text-3xl text-primary focus-visible:ring-accent transition-all" 
                       />
                    </div>
                 </div>
                 <div className="space-y-3">
                    ...
                    <div className="relative group">
                       <Coffee className="absolute left-5 md:left-6 top-1/2 -translate-y-1/2 h-4 w-4 text-primary/20 group-focus-within:text-accent transition-colors" />
                       <Input 
                         type="number" 
                         value={breakMins} 
              ...
              <div className="p-6 md:p-8 bg-slate-50/50 rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-inner relative overflow-hidden group/target">
                 ...
                 <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                       ...
                       <p className="text-lg md:text-xl font-black text-primary italic tracking-tighter uppercase leading-none">12 SEANS / 300 XP</p>
                    </div>
                 </div>
                 <div className="h-14 w-14 md:h-16 md:w-16 rounded-full bg-white flex items-center justify-center shadow-lg border-4 border-slate-50 font-black text-xs text-primary relative z-10">75%</div>
              </div>
           </Card>
           ...
           <Card className="p-8 md:p-10 rounded-[3.5rem] border-none shadow-xl bg-[#0F172A] text-white space-y-6 relative overflow-hidden group border-t-8 border-accent/20">
              ...
              <div className="flex items-center gap-4 relative z-10">
                 ...
                 <h4 className="text-lg md:text-xl font-black italic tracking-[0.2em] uppercase">AI ANALİZ</h4>
              </div>
              <p className="text-xl md:text-2xl font-medium opacity-60 italic relative z-10 leading-tight tracking-tight">
              ...
              <Card className="p-8 md:p-10 rounded-[4rem] border-none shadow-3xl bg-[#0F172A] text-white space-y-8 animate-in slide-in-from-right-8 duration-700 relative overflow-hidden">
                 ...
                 <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-4">
                       ...
                       <h4 className="text-lg md:text-xl font-black italic tracking-[0.3em] uppercase">SONIC FOCUS</h4>
                    </div>
                 </div>
                 ...
                 <div className="aspect-video w-full rounded-[2.5rem] bg-black/60 overflow-hidden relative shadow-3xl border border-white/5 group/video">
                    ...
                    <div className="absolute bottom-6 left-8 space-y-1">
                       ...
                       <p className="text-xl md:text-2xl font-black italic tracking-tighter uppercase line-clamp-1">{selectedStream.title}</p>
                    </div>
                 </div>
