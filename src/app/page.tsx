      ...
      <main className="pt-32 pb-20">
        <div className="container mx-auto px-6">
          <div className="flex flex-col items-center text-center space-y-8 md:space-y-12 max-w-5xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 pt-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500 w-full sm:w-auto">
          ...
          <div className="container mx-auto grid lg:grid-cols-2 gap-20 items-center relative z-10">
            <div className="space-y-10 text-white text-center lg:text-left">
               ...
               <div className="grid grid-cols-2 gap-12 pt-10 border-t border-white/5">
                  <div>
                     <p className="text-5xl md:text-7xl font-black text-accent text-shadow-accent">%94</p>
                     <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 italic mt-2">ODAK SKORU</p>
                  </div>
                  <div>
                     <p className="text-5xl md:text-7xl font-black text-white text-shadow-deep">25dk</p>
                     <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 italic mt-2">İDEAL SEANS</p>
                  </div>
               </div>
            </div>
        ...
        <section className="container mx-auto px-6 py-40">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {[
                ...
                <div key={i} className="p-12 bg-white rounded-[3.5rem] border border-primary/5 shadow-xl hover:shadow-2xl transition-all duration-500 group text-center md:text-left hover:-translate-y-3">
                   <div className="h-20 w-20 rounded-[1.75rem] bg-slate-50 flex items-center justify-center mb-10 shadow-inner group-hover:scale-110 group-hover:rotate-6 transition-all mx-auto md:ml-0">
                      <f.icon className={cn("h-10 w-10", f.iconColor || "text-primary")} />
                   </div>
                   <h3 className="text-2xl md:text-3xl font-black italic tracking-tighter text-primary uppercase mb-4">{f.title}</h3>
                   <p className="text-lg font-medium text-muted-foreground italic leading-relaxed opacity-70">{f.desc}</p>
                </div>
              ))}
           </div>
        </section>
