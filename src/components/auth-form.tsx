...
export function AuthForm({
  ...
  if (authMode === 'login' && !isProfileCompletion) {
    return (
      <div className="max-w-md mx-auto space-y-8 md:space-y-12 animate-in fade-in duration-700 px-4 w-full">
        <div className="text-center space-y-3">
           <h2 className="text-3xl md:text-5xl font-black italic tracking-tighter text-primary uppercase text-shadow-deep">SİSTEME DÖN</h2>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-10 md:space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-1000 px-4 md:px-6 w-full">
      <form onSubmit={handleAction} className="space-y-10 md:space-y-16">
        <div className="space-y-6 md:space-y-10">
          <Label className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.5em] text-muted-foreground/60 block text-center italic">ROLÜNÜZÜ BELİRLEYİN</Label>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={cn(
                    'p-3 md:p-10 rounded-[1.25rem] md:rounded-[2.5rem] border-2 transition-all duration-500 flex flex-col items-center gap-2 md:gap-5 group',
                ...
                >
                  <Icon className={cn('h-5 w-5 md:h-10 md:w-10 transition-transform group-hover:scale-110', role === r.id ? 'text-accent' : 'text-primary')} />
                  <span className="font-black text-[6px] md:text-[9px] tracking-[0.2em] md:tracking-[0.3em] uppercase text-center">{r.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="space-y-6 md:space-y-8">
          <div className="space-y-2 md:space-y-4">
             <Label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/50 ml-6 italic">TAM AD SOYAD *</Label>
          ...
          {!isProfileCompletion && (
             <div className="space-y-2 md:space-y-4">
                <Label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/50 ml-6 italic">SİSTEM E-POSTASI *</Label>
                <div className="relative group">
          ...
          {role === 'student' ? (
             <div className="space-y-2 md:space-y-4">
                <Label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-accent ml-6 italic">EĞİTMEN KODU (OPSİYONEL)</Label>
                <div className="relative group">
                   ...
                   <Input 
                     ...
                     className="h-14 md:h-20 rounded-[1.25rem] md:rounded-[2rem] bg-white border-none shadow-[0_30px_60px_-10px_rgba(0,0,0,0.05)] shadow-inner pl-14 md:pl-16 font-black tracking-[0.15em] md:tracking-[0.3em] text-sm md:text-lg uppercase focus-visible:ring-accent text-primary" 
            ...
            <div className="space-y-2 md:space-y-4">
               <Label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/50 ml-6 italic">KURUM ADI *</Label>
               <div className="relative group">
          ...
          {!isProfileCompletion && (
            <div className="space-y-2 md:space-y-4">
               <Label className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/50 ml-6 italic">GÜVENLİ ŞİFRE *</Label>
               <div className="relative group">
        ...
        {role === 'student' && (
           <div className="space-y-6 md:space-y-10">
              <Label className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.5em] text-muted-foreground/60 block text-center italic">AKADEMİK HEDEFİNİZİ SEÇİN</Label>
              <div className="bg-[#F8FAFC]/50 rounded-[2rem] md:rounded-[4rem] p-4 md:p-12 border-2 border-white shadow-2xl relative overflow-hidden">
                 <div className="absolute top-0 right-0 w-32 md:w-48 h-32 md:h-48 bg-accent/5 blur-3xl md:blur-[80px] rounded-full" />
                 <ScrollArea className="h-[250px] md:h-[400px] pr-2 md:pr-8">
                    <div className="space-y-8 md:space-y-12">
                       {Object.entries(categorizedExams).map(([cat, exams]) => exams.length > 0 && (
                          <div key={cat} className="space-y-3 md:space-y-6">
                             <h4 className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-primary/30 ml-4 md:ml-6 italic">{cat}</h4>
                             <div className="grid gap-2 md:gap-4">
                                {exams.map((exam) => (
                                   <button
                                     ...
                                     className={cn(
                                       "flex items-center justify-between p-4 md:p-8 rounded-[1.25rem] md:rounded-[2.5rem] border-2 transition-all duration-500 text-left group/item",
                                   ...
                                   >
                                      <div className="flex items-center gap-3 md:gap-8">
                                         <div className={cn("h-10 w-10 md:h-16 md:w-16 rounded-[0.75rem] md:rounded-[1.5rem] bg-slate-50 flex items-center justify-center shadow-inner group-hover/item:rotate-6 transition-all", targetExam === exam.id ? "bg-accent/10" : "")}>
                                            <Target className={cn("h-4 w-4 md:h-7 md:w-7", targetExam === exam.id ? "text-accent" : "text-primary/20")} />
                                         </div>
                                         <div className="space-y-0.5 md:space-y-1">
                                            <span className={cn("font-black text-xs md:text-lg uppercase tracking-tight block leading-none", targetExam === exam.id ? "text-primary" : "text-primary/60")}>{exam.title}</span>
                                            <span className="text-[6px] md:text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.1em] md:tracking-[0.2em]">{exam.targetGroup}</span>
                                         </div>
                                      </div>
                                      {targetExam === exam.id && <CheckCircle className="h-4 w-4 md:h-8 md:w-8 text-accent animate-in zoom-in-50" />}
                                   </button>
                                ))}
                             </div>
                          </div>
