      ...
      <section className="bg-primary text-white rounded-[3.5rem] p-8 md:p-14 relative overflow-hidden group shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="space-y-6 flex-1">
            ...
            <h1 className="text-3xl md:text-5xl font-black italic leading-[0.95] tracking-tighter uppercase">
               Hoş Geldin, bugün seni {currentDayPlan?.blocks?.length || 0} akademik blok bekliyor.
            </h1>
          </div>
          <Button onClick={() => router.push('/dashboard/planning')} className="w-full md:w-auto bg-accent hover:bg-white text-primary transition-all rounded-[2rem] h-20 px-12 font-black uppercase text-[12px] tracking-[0.3em] shadow-3xl">AKADEMİK TAKVİM</Button>
        </div>
      </section>
                ...
                <Card key={block.id} className={cn("p-8 rounded-[3.5rem] border-none shadow-xl transition-all hover:scale-[1.02] bg-white h-full flex flex-col", block.status === 'done' && "opacity-60")}>
                   <div className="space-y-6 flex-1 flex flex-col">
                           ...
                           <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 space-y-4">
                              <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                                 ...
                                 <div className="flex gap-2">
                                    {block.youtubeUrl && <a href={block.youtubeUrl} target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all"><Youtube className="h-4 w-4" /></a>}
                                    {block.pdfUrl && <a href={block.pdfUrl} target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all"><FileText className="h-4 w-4" /></a>}
                                    {block.mebiUrl && <a href={block.mebiUrl} target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all"><BookOpen className="h-4 w-4" /></a>}
                                 </div>
                              </div>
                        ...
                        <div className="flex justify-between gap-2 pt-6 border-t border-slate-50 mt-auto">
                           <Button onClick={() => handleTaskAction(block.id, 'done')} className={cn("flex-1 h-12 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all", block.status === 'done' ? "bg-slate-100 text-slate-400" : "bg-emerald-500 text-white shadow-xl")}>
                              <CheckCircle2 className="h-4 w-4 mr-2" /> {block.status === 'done' ? 'GERİ AL' : 'TAMAMLA'}
                           </Button>
                           <Button onClick={() => router.push('/dashboard/planning')} variant="outline" size="icon" className="h-12 w-12 rounded-xl border-slate-100 hover:border-primary text-primary transition-all"><Edit3 className="h-4 w-4" /></Button>
                        </div>
                   </div>
                </Card>
             ))}
