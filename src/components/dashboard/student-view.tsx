          ...
          <div key={dayPlan.date} className="space-y-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-4">
                 <div className="flex items-center gap-6">
                    <h2 className="text-3xl md:text-4xl font-black italic tracking-tighter text-primary uppercase text-shadow-deep">
                 ...
                 <Badge className="bg-white text-primary border-2 border-slate-100 rounded-3xl px-8 py-3.5 font-black uppercase text-[10px] tracking-[0.2em] shadow-lg">
                    <CalendarDays className="h-4 w-4 mr-2 text-accent" /> {format(parseISO(dayPlan.date), 'd MMMM yyyy', { locale: tr })}
                 </Badge>
            </div>
                    ...
                    <Card key={block.id} className={cn("p-10 rounded-[4rem] border-none transition-all hover:scale-[1.02] shadow-xl group relative overflow-hidden bg-white h-full flex flex-col", block.status === 'done' && "opacity-60")}>
                       <div className="space-y-8 relative z-10 flex-1 flex flex-col">
                            ...
                            <div className="flex justify-between gap-2 pt-6 border-t border-slate-50 mt-auto">
                               <Button onClick={() => handleTaskAction(dayPlan.date, block.id)} className={cn("flex-1 h-12 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all", block.status === 'done' ? "bg-slate-100 text-slate-400 shadow-inner" : "bg-emerald-500 text-white shadow-xl shadow-emerald-500/20")}>
