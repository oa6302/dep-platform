...
import { 
  Users, School, Plus, Copy, AlertTriangle, Brain, FileText, 
  LayoutDashboard, Sparkles, ArrowRight, CheckCircle2, 
  BookOpenCheck, Loader2, Save, Trash2, Edit3, Layers, Eye, Key,
  ChevronRight, TrendingUp, Calendar, MessageSquare
} from 'lucide-react';
    ...
    <div className="p-8 lg:p-12 space-y-12 max-w-7xl mx-auto w-full animate-in fade-in duration-1000">
      ...
      <Tabs defaultValue="overview" className="space-y-12">
        <TabsList className="bg-[#F1F5F9]/80 backdrop-blur-xl p-2.5 rounded-[3rem] h-24 shadow-inner flex overflow-x-auto scrollbar-hide border border-primary/5">
          ...
          <TabsTrigger value="requests" className="rounded-[2.5rem] px-12 h-full font-black text-xs uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-xl transition-all duration-500 gap-4 group relative">
             <MessageSquare className="h-5 w-5 group-data-[state=active]:text-accent" /> İstekler
              ...
              <Card className="xl:col-span-8 rounded-[4rem] border-none shadow-xl bg-white p-12 space-y-10 border border-primary/5">
                 ...
                 <div className="h-[300px] flex items-end gap-8 pb-4">
                    {[45, 68, 85, 52, 98, 74, 88].map((h, i) => (
                      <div key={i} className="flex-1 bg-slate-50 rounded-[2rem] relative group/bar transition-all">
              ...
              <div className="xl:col-span-4 space-y-10">
                 <Card className="rounded-[3.5rem] border-none shadow-xl bg-primary text-white p-12 space-y-8 relative overflow-hidden group">
                    ...
                    <p className="text-lg leading-relaxed font-medium opacity-90 italic">
                       "Bağlı olduğunuz 14 öğrencinin 2'si Matematik netlerinde düşüş yaşıyor. Simülasyon modunda eksiklerini kontrol etmeniz önerilir."
                    </p>
                    <Button className="w-full h-16 rounded-2xl bg-accent hover:bg-white hover:text-primary transition-all font-black text-xs uppercase tracking-widest shadow-2xl">Raporu Gör</Button>
                 </Card>
              </div>
           </div>
        </TabsContent>
                 ...
                 <DialogContent className="rounded-[3rem] border-none shadow-2xl p-10 bg-white">
                    ...
                    <form onSubmit={handleAddClassroom} className="space-y-8 pt-8">
                       <div className="space-y-3">
                          <Label className="text-xs font-black uppercase tracking-widest opacity-40 ml-2 italic">Şube Adı</Label>
                       ...
                       <div className="space-y-3">
                          <Label className="text-xs font-black uppercase tracking-widest opacity-40 ml-2 italic">Kademe / Sınıf</Label>
                       ...
                       <Button type="submit" disabled={loading === 'add-class'} className="w-full h-18 rounded-[1.75rem] bg-primary hover:bg-accent font-black text-xs uppercase tracking-widest gap-3 shadow-2xl shadow-primary/20">
                 ...
                 <Card key={cls.id} className="premium-card p-10 group border border-primary/5 relative overflow-hidden">
                    <div className="space-y-8">
                       ...
                       <div className="flex gap-4 pt-2">
                          <Button variant="outline" className="flex-1 h-12 rounded-xl border-2 font-black text-[10px] uppercase tracking-widest">Yönet</Button>
                          <Button size="icon" className="h-12 w-12 rounded-xl bg-slate-50 text-primary hover:bg-primary hover:text-white transition-all"><Settings className="h-5 w-5" /></Button>
                       </div>
                    </div>
                 </Card>
              ))}
              ...
              <Dialog open={isAddingSubject} onOpenChange={setIsAddingSubject}>
                 ...
                 <DialogContent className="rounded-[3rem] border-none shadow-2xl p-10 bg-white">
                    ...
                    <form onSubmit={handleAddSubject} className="space-y-8 pt-8">
                       <div className="space-y-3">
                          <Label className="text-xs font-black uppercase tracking-widest opacity-40 ml-2 italic">Program (Sınav)</Label>
                       ...
                       <div className="space-y-3">
                          <Label className="text-xs font-black uppercase tracking-widest opacity-40 ml-2 italic">Ders Adı</Label>
                 ...
                 <Card key={student.id} className="premium-card p-12 group border border-primary/5 relative overflow-hidden">
                    ...
                    <div className="space-y-8">
                       ...
                       <div>
                          ...
                          <p className="text-[11px] font-black text-accent uppercase tracking-widest">{student.targetExam} • {student.grade || 'Hazırlık'}</p>
                       </div>
                       <div className="flex gap-4">
                          <Button onClick={() => router.push(`/dashboard?simulate=${student.id}`)} size="icon" className="h-14 w-14 rounded-2xl bg-primary hover:bg-accent text-white shadow-xl transition-all" title="Simülasyon Modu">
        ...
        <TabsContent value="requests" className="outline-none animate-in fade-in">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {requests.map((req) => (
                 <Card key={req.id} className="premium-card p-10 border border-primary/5 bg-white relative overflow-hidden">
                    <div className="flex items-center gap-8">
                       <div className={cn(
                          "h-16 w-16 rounded-2xl flex items-center justify-center text-white shadow-xl",
                          req.type === 'code_request' ? 'bg-accent' : 'bg-primary'
                       )}>
                          {req.type === 'code_request' ? <Key className="h-8 w-8" /> : <Users className="h-8 w-8" />}
                       </div>
                       <div className="flex-1">
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 italic">
                             {req.type === 'code_request' ? 'AKTİVASYON KODU TALEBİ' : 'BAĞLANTI İSTEĞİ'}
                          </p>
                          <p className="text-xl font-black text-primary tracking-tight">{req.message}</p>
                       </div>
                    </div>
                    <div className="flex gap-4 pt-8">
                       <Button className="flex-1 h-14 rounded-xl bg-emerald-500 hover:bg-emerald-600 font-black text-[10px] uppercase tracking-widest">Onayla</Button>
                       <Button variant="outline" className="flex-1 h-14 rounded-xl border-2 font-black text-[10px] uppercase tracking-widest text-destructive hover:bg-destructive/5 hover:border-destructive/20">Reddet</Button>
                    </div>
                 </Card>
              ))}
              {requests.length === 0 && (
                 <div className="col-span-full py-40 text-center opacity-30 italic font-black uppercase tracking-widest text-xs">Henüz bekleyen bir talep bulunmuyor.</div>
              )}
           </div>
        </TabsContent>
      </Tabs>
